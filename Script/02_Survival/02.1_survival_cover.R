...
source("Script/01a_Survival_Functions/cover_lrtest_function.R")
...
survival_cover_models$model_0
survival_cover_models$model_1
survival_cover_models$model_2
survival_cover_models$model_3
survival_cover_models$model_c


# Saving models as a RDS file --------------------------------------------------

saveRDS(survival_cover_models, file = here("Data/04_Temp", paste0(Sys.Date(), "_survival_cover_models_dataframe_Bv1.rds")))

# Calling RDS files ------------------------------------------------------------

survival_cover_models_dataframe <- readRDS(file = here("Data/04_Temp", "2024-02-02_survival_cover_models_dataframe_Bv1.rds"))

survival_cover_models_dataframe

# Testing models ---------------------------------------------------------------

# Expanding the lists 

loc_list <- unique(survival_cover_models_dataframe$location)

SC_models <- subset(survival_cover_models_dataframe, select = -c(data, location))
...
write.csv(SH_p_vals, file = here("Data/05_Output", paste0(Sys.Date(), "_Survival_Cover_pvals_Bv1.csv")), 
          row.names = FALSE)

write.csv(SH_p_vals, file = here("Data/05_Output", paste0(Sys.Date(), "_Survival_Cover_sig_pvals_Bv1.csv")), 
          row.names = FALSE)
