...
survivalCover_2B <- function(df) {
  ...
  for (var in ClimaticVarList) {
    # Perform the regression
    model <- glmer(paste("survival ~", var, paste(" + tree_cover + (1|blockF/plotF/splitplotF)")), 
                   data = df, family = binomial, 
                   control = glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 100000)))
    # Store the results in the list
    results[[var]] <- model
...
survivalCover_3B <- function(df) {
  ...
  for (var in ClimaticVarList) {
    # Perform the regression
    model <- glmer(paste("survival ~", var, paste("+ tree_cover +"), var, paste(" * tree_cover + (1|blockF/plotF/splitplotF)")), 
                   data = df, family = binomial, 
                   control = glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 100000)))
    # Store the results in the list
    results[[var]] <- model
