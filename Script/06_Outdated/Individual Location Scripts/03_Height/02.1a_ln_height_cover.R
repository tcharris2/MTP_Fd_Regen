...
# Create a new tibble 
ln_height_cover_models <- ln_height_loc_group_cover

# Run the model functions with mutate to store them 

# Stores the null model as Model_0
ln_height_cover_models <- ln_height_cover_models %>% 
...
ln_height_cover_models$model_0
ln_height_cover_models$model_1
ln_height_cover_models$model_2
ln_height_cover_models$model_3
ln_height_cover_models$model_c
...
ln_height_cover_model_0_fits <- subset(ln_height_cover_model_0_resid, select = c("location", "data"))

ln_height_cover_model_0_fits <- unnest(ln_height_cover_model_0_fits)
ln_height_cover_model_0_fits

# Graphing
ggplot(data = ln_height_cover_model_0_fits) +
...
write.csv(HC_p_vals, file = here("Data/05_Output", paste0(Sys.Date(), "_ln_Height_Cover_pvals_NoFutures.csv")), 
          row.names = FALSE)

write.csv(HC_sig_p_vals, file = here("Data/05_Output", paste0(Sys.Date(), "_ln_Height_Cover_sig_pvals_NoFutures.csv")), 
          row.names = FALSE)
