...
heightHarvest <- function(df) {
  
  lmer(height ~ harvestF + (1|plotF/splitplotF), data = df,
       REML = FALSE,
       control = lmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 100000)))
}
...
heightHarvest_2 <- function(df) {
  ...
  for (var in ClimaticVarList) {
    # Perform the regression
    model <- lmer(paste("height ~", var, paste(" + harvestF + (1|plotF/splitplotF)")), 
                   data = df, REML = FALSE,
                  control = lmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 100000)))
    # Store the results in the list
    results[[var]] <- model
...
heightHarvest_3 <- function(df) {
  ...
  for (var in ClimaticVarList) {
    # Perform the regression
    model <- lmer(paste("height ~", var, paste("+ harvestF +"), var, paste(" * harvestF + (1|plotF/splitplotF)")), 
                   data = df, REML = FALSE,
                  control = lmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 100000)))
    # Store the results in the list
    results[[var]] <- model
